...
class DataIngestion:
    ...
    def process_bbox(self):
        bbox_csv = pd.read(os.path.join(self.config.root_dir, 'train_solution_bounding_boxes.csv'))
        for idx, data in bbox_csv.groupby('image'):
            image_path = os.path.join(self.config.root_dir, idx)
            try:
                ...
                if idx in list_of_images:
                    with open(os.path.join(self.config.root_dir, f'{idx.replace(".jpg", "")}.txt'), 'w+') as f:
